...
import { badges, rewardTier } from '../data.js';

function BadgeTile({ badge }) {
  const BadgeIcon = Icon[badge.icon];
  return (
...
export default function RewardsBadges() {
  const pct = Math.round((rewardTier.points / rewardTier.pointsToNext) * 100);
        ...
        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{rewardTier.points.toLocaleString('en-IN')} pts · {rewardTier.pointsToNext - rewardTier.points} pts to {rewardTier.nextTier}</p>
            </div>
          </div>
          <div className="w-full sm:w-64">
            <ProgressBar value={pct} fillClassName="bg-amber-500" />
            <p className="mt-1 text-right text-[11px] text-slate-400">{pct}% to {rewardTier.nextTier}</p>
          </div>
        </div>
      </Card>
