...
import { allCampaigns } from '../data.js';
...
function CampaignCard({ campaign }) {
  return (
    <Card>
      <div className={`relative h-32 w-full overflow-hidden rounded-lg bg-gradient-to-br ${campaign.theme}`}>
      ...
      <p className="text-xs text-slate-500">{campaign.org}</p>
      ...
      <div className="mt-1.5 flex items-center justify-between text-[11px]">
        ...
        <span className="text-slate-400">₹{campaign.raised.toLocaleString('en-IN')} / ₹{campaign.goal.toLocaleString('en-IN')}</span>
      </div>

      <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-slate-500">
        <span className="flex items-center gap-1"><Icon.Users className="h-3.5 w-3.5" />{campaign.donors} donors</span>
